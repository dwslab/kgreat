...
def process_action(kg_name: str, stage: str, action: str, steps: List[str], container_manager: str):
    if stage == 'mapping':
        _perform_mapping_action(kg_name, stage, action, steps, container_manager)
    elif stage == 'preprocessing/embeddings':
        _perform_embedding_action(kg_name, stage, action, steps, container_manager)
    elif stage == 'tasks':
        _perform_task_action(kg_name, stage, action, steps, container_manager)
    else:
        raise NotImplementedError(f'Stage not implemented: {stage}')


def _perform_mapping_action(kg_name: str, stage: str, action: str, steps: List[str], container_manager: str):
    if action == 'prepare':
...
def _perform_embedding_action(kg_name: str, stage: str, action: str, steps: List[str], container_manager: str):
...
def _perform_task_action(kg_name: str, stage: str, action: str, steps: List[str], container_manager: str):
...
if __name__ == "__main__":
    ...
    parser.add_argument('stage', type=str, help='Apply action to this stage', choices=['mapping', 'preprocessing/embeddings', 'tasks'])
    ...
    process_action(args.kg_name, args.stage, args.action, args.step, args.container_manager)
